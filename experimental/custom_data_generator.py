...
class TrainValidTestGenerator():
    ...
    def __init__(self, data_dir, label_dir, pair_csv,
        ...
        self._generators = {
            'train': DataGenerator(train_frame, transformation=transformation[0], image_shape=image_shape, mask_shape=mask_shape, augment_time=augment_time, batch_size=batch_size, is_shuffle=is_shuffle, self_supervised=self_supervised, self_reconstruct=self_reconstruct,is_train=True),
            'valid': DataGenerator(valid_frame, transformation=transformation[1], image_shape=image_shape, mask_shape=mask_shape, augment_time=augment_time, batch_size=batch_size, is_shuffle=is_shuffle, self_supervised=self_supervised, self_reconstruct=self_reconstruct, is_train=True),
            'test':  DataGenerator(test_frame,  transformation=transformation[2], image_shape=image_shape, mask_shape=mask_shape, augment_time=augment_time, batch_size=batch_size, is_shuffle=is_shuffle, self_supervised=self_supervised, self_reconstruct=self_reconstruct,is_train=True)}

    def set_train_status(self, is_train):
        self._generators['train'].set_train_status(is_train)
        self._generators['valid'].set_train_status(is_train)
        self._generators['test'].set_train_status(is_train)

    def set_transform(self, generator_type, transform):
        self._generators[generator_type].transform=transform

    def get_transform(self, generator_type):
        return self._generators[generator_type].transform
